...
export const AdminSystemSettings: React.FC = () => {
    ...
    return (
        <div className="space-y-6 max-w-[900px]">
            ...
            <Card className="rounded-2xl border-slate-200 shadow-sm" title={<span className="text-sm font-black text-slate-800 flex items-center gap-2"><RobotOutlined /> FinBERT 中文金融情感</span>} extra={<Tag color={enabled ? 'success' : 'default'} className="m-0 border-none text-[11px]">{enabled ? '已开启' : enabled === null ? '未知' : '已关闭'}</Tag>}>
                <div className="flex items-center justify-between">
                    ...
                    <Space direction="vertical" align="center" size={4}>
                        <Switch checked={!!enabled} loading={loading || enabled === null} onChange={handleToggle} checkedChildren="开启" unCheckedChildren="关闭" />
                        <Text className="text-[11px] text-slate-400">{enabled ? '运行中' : '已停用'}</Text>
                    </Space>
                </div>
                <Divider className="!my-4" />
                <Alert type="info" showIcon className="rounded-xl text-xs" message="提示" description="首次开启需后台加载模型约 10-20s，期间新入库资讯仍走字典法；已入库历史需重建 enrichment 才会回填 FinBERT 置信度。" />
            </Card>

            <Card className="rounded-2xl border-dashed border-slate-200 bg-slate-50/50" title={<span className="text-sm font-bold text-slate-500">更多系统设置（占位）</span>}>
