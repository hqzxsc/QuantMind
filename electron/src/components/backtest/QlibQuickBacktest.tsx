...
/** 自定义股票池弹窗行 */
const CustomPoolRow: React.FC<{ pool: StockPoolOption; selected: boolean; onSelect: () => void }> = ({
  pool,
  selected,
  onSelect,
}) => (
  <button
    type="button"
    onClick={onSelect}
    className={`w-full text-left px-3 py-2.5 rounded-xl border mb-2 transition-all flex items-center gap-3 ${
      selected
  ...
  >
    <div className="flex-1 min-w-0">
      <div className="flex items-center gap-2">
        <span className="text-[13px] font-bold text-slate-800 truncate">{pool.name}</span>
        {!pool.is_system && (
          <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-emerald-50 text-emerald-600 shrink-0">自定义</span>
        )}
      </div>
      <div className="text-[11px] text-slate-400 font-mono truncate mt-0.5">
        {pool.code} · {pool.symbol_count} 只{pool.checksum ? ` · ${pool.checksum.slice(0, 8)}` : ''}
      </div>
    </div>
...
export const QlibQuickBacktest: React.FC = () => {
  ...
  // 自定义股票池（中证1000按钮 → 弹窗选择，全局股票池只读接口）
  const [customPoolOpen, setCustomPoolOpen] = useState(false);
  const [customPools, setCustomPools] = useState<StockPoolOption[]>([]);
  const [customPoolsLoading, setCustomPoolsLoading] = useState(false);
  const [customPoolLoadError, setCustomPoolLoadError] = useState('');
  ...
  const openCustomPools = async () => {
    setCustomPoolOpen(true);
    setCustomPoolsLoading(true);
    setCustomPoolLoadError('');
    try {
      const items = await listStockPoolOptions({ market: 'CN' });
      setCustomPools(items);
    } catch (e) {
      setCustomPoolLoadError(e instanceof Error ? e.message : '股票池列表加载失败');
    } finally {
      setCustomPoolsLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      {customPoolOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setCustomPoolOpen(false)}>
          <div
          ...
          >
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <div>
                <div className="text-sm font-bold text-slate-800">自定义股票池</div>
                <div className="text-[11px] text-slate-500 mt-0.5">全局股票池（后台维护，保存即生效）· 选择后回测按池成分执行</div>
              </div>
              <button type="button" onClick={() => setCustomPoolOpen(false)} className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto custom-scrollbar p-3">
              {customPoolsLoading ? (
                ...
                <div className="py-10 text-center text-sm text-red-500">{customPoolLoadError}</div>
                ...
                <>
                  {customPools.filter((p) => !p.is_system).map((pool) => (
                    <CustomPoolRow key={pool.pool_id} pool={pool} selected={selectedCustomPool?.pool_id === pool.pool_id} onSelect={() => selectCustomPool(pool)} />
                  ))}
                  {customPools.some((p) => p.is_system) && (
