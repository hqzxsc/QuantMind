...
```bash
python3 <repo>/.claude/skills/stock-picks/scripts/pick_candidates.py --data-date 20260821 --window 3 --top 30 --json   # 跨3日聚合
python3 <repo>/.claude/skills/stock-picks/scripts/pick_candidates.py --data-date 20260821 --window 1 --top 30 --json   # 严格单日（无未来视觉）
python3 <repo>/.claude/skills/stock-picks/scripts/pick_candidates.py --top 30 --json   # 默认最近全量推理日
python3 <repo>/.claude/skills/stock-picks/scripts/pick_candidates.py --top 30 --json --no-l2  # 跳过 L2（更快）
```
...
docker cp 选股推荐.md quantmind:/app/db/trading_agents_results/每日选股/每日选股推荐_2026-08-21.md
docker cp 选股推荐.pdf quantmind:/app/db/trading_agents_results/每日选股/每日选股推荐_2026-08-21.pdf
```
...
- 改 SKILL.md/脚本后必须同步双副本：`cp -r .claude/skills/stock-picks ~/.claude/skills/`
