    ...
    <div className="space-y-4">
        ...
        <Collapse
          ...
          items={[{
            ...
            children: <div className="text-xs text-slate-600 leading-relaxed space-y-1">
              <div>均分 = 当日 Top100 高分 `pred/fusion_score` 均值（与个股终端一致，回退读 `pred.parquet`），`牛市 ≥0.08 红实线 / 震荡 0.02-0.08 灰 / 熊市 &lt;0.02 绿虚线`。</div>
              <div>仅最近 90 交易日，`y` 域 `[-0.15,0.15]`，Top100 兼顾广度与灵敏度。</div>
            </div>,
          }]}
        />
      </Card>
      ...
      <Card className="rounded-2xl" size="small">
        <div className="flex items-center gap-2 mb-2">
          ...
          <Text className="text-xs font-black text-slate-700">Top100 均分时序（横线分区）</Text>
        </div>
